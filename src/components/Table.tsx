    ...
    <div className={cn(
      ...
      <table className="w-full text-[13px] leading-tight text-left border-collapse font-sans min-w-full">
        ...
        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
          {isLoading ? (
            <tr>
              <td colSpan={columns.length} className="px-4 py-10 text-center text-slate-400 dark:text-slate-500 font-medium">
            ...
            data.map((item, rowIdx) => (
              <tr
                ...
                className={cn(
                  ...
                  rowIdx % 2 === 0 ? "bg-white dark:bg-slate-900" : "bg-slate-50/50 dark:bg-slate-950/30",
                  onRowClick ? "cursor-pointer hover:bg-primary/5" : "hover:bg-slate-50 dark:hover:bg-slate-800/50"
                )}
              >
