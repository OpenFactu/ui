...
export const Toast: React.FC<ToastProps> = ({ id, message, type = 'info', onClose }) => {
  ...
  const borderClasses = {
    success: 'border-emerald-200 bg-white/90 shadow-emerald-500/5',
    error: 'border-rose-200 bg-white/90 shadow-rose-500/5',
    info: 'border-blue-200 bg-white/90 shadow-blue-500/5',
    warning: 'border-amber-200 bg-white/90 shadow-amber-500/5',
  };
  ...
  return (
    <div className={cn(
      ...
      <div className="flex-shrink-0 w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center border border-slate-100 shadow-inner">
        {icons[type]}
      </div>
      ...
      <button 
        onClick={() => onClose(id)}
        className="flex-shrink-0 rounded-xl p-2 text-slate-300 hover:bg-slate-100 hover:text-slate-600 transition-all border border-transparent hover:border-slate-200"
      >
        <X size={16} />
      </button>
    </div>
